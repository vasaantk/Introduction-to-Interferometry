...
ax2.plot(xaxis,   sinc, c='r')    # Plot sinc       and give x-axis degrees
# ax2.axvline(firstNull, color='k', alpha=0.3)    # Plot first null on degrees' axis

ax1.set_title( 'sinc($\\frac{'+str(int(Dnu))+' }{'+str(int(nu))+'}$ * '+str(int(u))+' * $\ell$) * cos(2$\pi $ * '+str(int(u))+' * $\ell$)', y = 1.09, fontsize= textsize)
ax1.set_ylabel('Response from correlator to square bandpass (Left:  R$_c$ , Right:  R$_s$)', fontsize= textsize)
...
ax4.plot(xaxis,   sinc, c='r')    # Plot sinc       and give x-axis degrees
# ax4.axvline(firstNull, color='k', alpha=0.3)    # Plot first null on degrees' axis

ax4.set_title( 'sinc($\\frac{'+str(int(Dnu))+' }{'+str(int(nu))+'}$ * '+str(int(u))+' * $\ell$) * sin(2$\pi $ * '+str(int(u))+' * $\ell$)', y = 1.09, fontsize= textsize)
ax3.get_yaxis().set_ticklabels([])
ax3.tick_params(axis='both', which='both', labelsize= textsize)
ax4.tick_params(axis='both', which='both', labelsize= textsize)

# plt.savefig('05-fin-band-lecture2.eps', transparent=True, format='eps')
plt.show()
